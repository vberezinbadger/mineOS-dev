...
if _VERSION == "Lua 5.2" then
  print("openupt: Lua 5.2 is unsupported. Please use Lua 5.3 instead.")
  return false
end
...
function openupt.partitions() -- Return some info about partitions
  local rtn = {}
  for i=1, #ptable, 1 do
    table.insert(rtn, {guid = ptable[i][5], fstype = ptable[i][3], size = (ptable[i][2] - ptable[i][1]) * 512, label = ptable[i][7], start = ptable[i][1], ["end"] = ptable[i][2]})
  end
  return rtn
end
...
function openupt.mkpart(pstart, pend, ptype, flags, label)
  ...
  if #ptable == 8 then
    return print("openupt: Too many partitions")
  end
  if pstart < 33 then
    pstart = 33
  end

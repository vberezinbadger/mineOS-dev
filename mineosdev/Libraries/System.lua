...
function system.updateDesktop()
	...
	desktopMenuMineOSItem:addItem(localization.aboutSystem).onTouch = function()
		...
		local lines = {
			...
			" ",
			"Version: 3.0.010 (Developer Preview)",
			" ",
			"Powered on: mineCORE 3.0",
		}
	...
	desktopMenuMineOSItem:addItem(localization.mineshell).onTouch = function()
		system.execute('minecore.lua')
	end
	...
	desktopMenuMineOSItem:addItem(localization.minebash).onTouch = function()
		system.execute('/Applications/mineBASH.app/Main.lua')
	end

	desktopMenuMineOSItem:addItem(localization.runfinder).onTouch = function()
		system.execute('/Applications/Finder.app/Main.lua')
	end
...
function system.authorize()
	...
	if #userList > 1 or loadUserSettingsAndCheckProtection(userList[1]) then
		...
		addButton("Go to shell", function()
			system.execute('minecore.lua')
		end)
		...
		local function selectUser()
			local function securityCheck(userName)
				...
				local input = addBackgroundContainerInput(passwordContainer, "", "Enter password", false, "•")
